    ...
    <div className="space-y-6">
      ...
      {selectedPost && (
        <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-sm flex items-center justify-center p-4 z-50" onClick={() => setSelectedPost(null)}>
          <div className="bg-white rounded-3xl w-full max-w-3xl max-h-[90vh] shadow-2xl animate-scale-up flex flex-col md:flex-row overflow-hidden" onClick={e => e.stopPropagation()}>
            {selectedPost.image && (
              <div className="md:w-80 shrink-0 bg-slate-50 flex items-center justify-center overflow-hidden md:min-h-[400px]">
                <img
            ...
            <div className="flex-1 overflow-y-auto p-6 md:p-8 space-y-5">
              <div className="flex items-start justify-between gap-4">
                <div className="space-y-3 flex-1">
                  <span className={`inline-block text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full border ${getCategoryColor(selectedPost.category)}`}>
...
function BlogCard({ post, onClick }: { post: BlogPost; onClick: () => void }) {
  return (
    <button onClick={onClick} className="flex flex-col md:flex-row bg-white rounded-2xl border border-slate-100 shadow-xs overflow-hidden text-left hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200 group w-full">
      {post.image ? (
        ...
        <div className="md:w-56 md:h-44 shrink-0 bg-gradient-to-br from-[#eef4ff] to-[#dfe9fa] flex items-center justify-center">
      ...
      <div className="flex-1 p-5 space-y-2.5 flex flex-col justify-center">
        <div className="flex items-center gap-3">
          <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full border ${getCategoryColor(post.category)}`}>
        ...
        <h4 className="font-display font-bold text-sm md:text-base text-slate-900 leading-snug group-hover:text-[#00346f] transition-colors line-clamp-2">{post.title}</h4>
