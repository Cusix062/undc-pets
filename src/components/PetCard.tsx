...
export default function PetCard({ pet, onSelect, isAdmin, onEdit, onDelete }: PetCardProps) {
  ...
  return (
    ...
    >
      ...
      {isAdmin && (
        <div className="absolute top-3 right-3 z-20 flex gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
          <button
            ...
            className="bg-white/90 hover:bg-white text-[#00346f] rounded-full p-1.5 shadow-md backdrop-blur-xs"
          ...
          <button
            ...
            className="bg-white/90 hover:bg-white text-rose-600 rounded-full p-1.5 shadow-md backdrop-blur-xs"
      ...
      <div onClick={() => onSelect(pet)} className="flex flex-col h-full">
        {/* Pet Image Container */}
